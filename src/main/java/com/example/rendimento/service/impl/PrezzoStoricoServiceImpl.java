...
public class PrezzoStoricoServiceImpl implements PrezzoStoricoService {
    ...
    @Override
    public List<PrezzoStorico> estraiPrezziStorici(Titolo titolo, DayOfWeek giornoSettimana) throws Exception {
        List<PrezzoStorico> prezziStorici = new ArrayList<>();

        // Costruisci l'URL della pagina dei dati storici su Investing usando il nome del titolo
        ...
        log.info("Tentativo di accesso all'URL con nome titolo: {}", url);

        try {
            // Tenta di accedere alla pagina usando il nome del titolo
            prezziStorici = estraiPrezziDaUrl(url, titolo, giornoSettimana);
        } catch (HttpStatusException e) {
            // Se otteniamo un 404, proviamo con l'ISIN
            if (e.getStatusCode() == 404) {
                log.warn("URL non trovato (404) usando il nome del titolo. Tentativo con ISIN...");
                String isin = titolo.getCodiceIsin().toLowerCase();
                String urlIsin = "https://it.investing.com/rates-bonds/" + isin + "-historical-data";
                log.info("Tentativo di accesso all'URL con ISIN: {}", urlIsin);
                
                try {
                    prezziStorici = estraiPrezziDaUrl(urlIsin, titolo, giornoSettimana);
                } catch (Exception e2) {
                    log.error("Errore anche con l'approccio ISIN: {}", e2.getMessage());
                    throw new Exception("Impossibile recuperare i prezzi storici sia con nome che con ISIN", e2);
                }
            } else {
                // Se non è un 404, rilancia l'eccezione originale
                throw e;
            }
        } catch (Exception e) {
    ...
    /**
     * Metodo helper per estrarre i prezzi da un URL specifico
     */
    private List<PrezzoStorico> estraiPrezziDaUrl(String url, Titolo titolo, DayOfWeek giornoSettimana) throws Exception {
        List<PrezzoStorico> prezziStorici = new ArrayList<>();
    ...
    @Override
    public List<Map<String, Object>> estraiPrezziStoriciMap(Titolo titolo, DayOfWeek giornoSettimana) throws Exception {
        List<Map<String, Object>> risultato = new ArrayList<>();
        
        // Riutilizziamo il metodo esistente per evitare duplicazione di codice
        List<PrezzoStorico> prezziStorici = estraiPrezziStorici(titolo, giornoSettimana);
